...
- To add two matrices simply add the correspinding elements
...
$$\begin{align}y_1&=\beta_1x_{11}+\beta_2x_{12}+\ldots+\beta_px_{1p}+\epsilon_1\\\ y_2&=\beta_1x_{21}+\beta_2x_{22}+\ldots+\beta_px_{2p}+\epsilon_2\\\ \vdots&=\vdots\quad+\vdots\quad\quad+\ldots+\quad\vdots\quad+\vdots\\\ y_n&=\beta_1x_{n1}+\beta_2x_{n2}+\ldots+\beta_px_{np}+\epsilon_n\end{align}$$
---
...
- The matrix $\mathbf{X}$ is  $n\times p$
  - $x_{ij}$ is the $i^{th}$ observation of the $j^{th}$ dependent variable.  For an intercept set $x_{i1}=1$ for all $i$

---

# Regression Model

- The vector $\boldsymbol{\beta}$ is  $p\times 1$
...
$$s^2=\sum\limits_{i=1}^n\left[(y_i-\bar{y})^2\right]$$
...
$$\boldsymbol{S}=\sum\limits_{i=1}^n\left[(\mathbf{y}_i-\boldsymbol{\bar{y}})(\mathbf{y}-\boldsymbol{\bar{y}})'\right]$$
