...
- The $d_{ij}$ are high dimensional distances that come from the solution.
...
#oz<-get_map(location='Australia',zoom=4,source="osm",maptype = "watercolor",color = "color")
...
- Classical MDS rotates the points in high-dimensional space to get a 2D solution.
--

- We can further rotate the points within 2 dimensions.<!--D-->
--
...
- This specific problem is called *serialisation*.
...
df<-tibble(rank=xg,
           ...
           xrot=pca$x[,1],
           yrot=pca$x[,2])
ggplot(df,aes(x=x,y=y,col=rank))+geom_point()+scale_color_viridis_c()
```
...
```{r,echo=FALSE}
ggplot(df,aes(x=xrot,y=yrot,col=rank))+geom_point()+scale_color_viridis_c()
```
...
- The dark blue and yellow plots are represented as close to one another.
...
- By *invariant* we mean that the configuration provided by the Sammon mapping doesn't change.
