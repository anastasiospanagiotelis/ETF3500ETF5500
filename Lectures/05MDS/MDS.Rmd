...
- The above problem has a unique and tractable solution when Euclidean distance is used.
--

- This solution depends on an Eigenvalue decomposition.
--

- Imagine this solution as *rotating* the points until we get a 2D view that represents the true distances as accurately as possible.
...
- We can further rotate the points with 2 dimensions.<!--D-->
--
...
kBeer$points%>%
  as.data.frame()%>%
  rownames_to_column('BeerName')%>%
  ggplot(aes(x=V1,y=V2,label=BeerName))+
  geom_text(size=10)
```
