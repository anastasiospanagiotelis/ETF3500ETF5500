...
- You may have covered matrices before and found that it involves remembering many seemingly arbitrary rules.<!--D-->
--

- The aim of this lecture is to understand the *geometric* interpretation of vectors which is crucial in making the connection between data and visualisation.<!--D-->
--
...
```{r multpts2,  fig.align='center',echo=FALSE,fig.show='animate',interval=0.5,fig.cap='Click to see'}
grid2<-rbind(rnorm(50),rnorm(50))
s<-seq(1,0.5,by=-0.1)
for (j in 1:length(s)){
  plot(0,0,'n',xlim=c(-5.1,5.1),ylim=c(-5.1,5.1),xlab='',ylab='')
...
- You may have learnt about matrices before as an array of numbers.<!--D-->
--

- We will learn about matrices as objects that *transform* points in space.<!--D-->
--
...
- If you have previously seen a matrix you have probably seen something like this
...
A diagonal matrix only has non-zeros running from the top left to the bottom right
...
This will stretch space horizontally and squeeze space vertically.

---

# Scaling Space

```{r matmults,  fig.align='center',echo=FALSE,fig.show='animate',interval=0.5,aniopts='controls', fig.cap='Click to see',ffmpeg.format='ogg'}
g=10
...
This matrix leaves all points in the same place
...
- It spins all the points around keeping them the same distance from the origin.<!--D-->
--
...
- Recall when in PCA we spun around the 3d scatterplot.
- The same idea works for bigger matrices
...
- Here $\mathbf{R'}$ is called the transpose.<!--D-->
--

- The transpose involves <!--D-->
--

  + Taking all rows of a matrix and converting them to columns
  + Taking all columns of a matrix and converting them to rows<!--D-->
--

- You will see more examples in the tutorial.
...
- What does it mean to multiply matrices by matrices?<!--D-->
--

- One way to understand matrix multiplication is as a *composition* of transformations:<!--D-->
--

  + Transform a $2$-dimensional vector $\mathbf{v}$ into a $4$-dimensional vector by multiplying by the $4\times 2$ matrix $\mathbf{G}$<!--D-->
--

  + Transorm the result into a $3$-dimensional vector by multyplying by a $3\times 4$ matrix $\mathbf{F}$
...
- A vector can be more than just a point in space.<!--D-->
--

- Also a matrix becomes something more than just something that moves points space.<!--D-->
--

- Matrices can store information too:<!--D-->
--

  + The data can be put in a matrix<!--D-->
--

  + The variances and covariances can be put into a matrix

---
  
# Where to now?
  
- In tutorials you will reinforce these concepts<!--D-->
--

- There will be more of a focus on matrix algebra:<!--D-->
--

  + The rules of matrix multiplication (with exercises)<!--D-->
--

  + The concept of a transpose will also be covered more thoroughly<!--D-->
--

- In next week's lecture we link these ideas back to PCA, MDS and CA.
