...
- Last week we looked at the concept of *distance* between observations.<!--D-->
--
...
- Can we see higher dimensional distances or non Euclidean distances in 2-dimensions?<!--D-->
--
...
- Difference between the 3D distance of reality, and the 2D distance in the photo.<!--D-->
--
...
```{r beer, echo=FALSE,fig.width=8,fig.height=8}
load('Beer.RData') #Make sure you change this to reflect your directory structure
...
```{r getplot, fig.height=4,echo=TRUE}
mdsout%>%
  as_data_frame%>% 
...
```{r getplotn, fig.height=4, echo=TRUE}
mdsout%>%
...
- There are some similarities with PCA<!--D-->
--

  + We represent information in a small number of dimensions<!--D-->
--

  + This representation involves some sort of rotation in the high dimensional space.<!--D-->
--

- In the case where the $\delta$ are Euclidean distances MDS will be equivalent to PCA.<!--D-->
--

- For this reason, the coordinates that come out of classical MDS are
often called **principal coordinates**.
---

# MDS and PCA

- This also means that when we look at principal components we know two things<!--D-->
--

  + The principal components maximise the explained variance<!--D-->
--
...
- To evaluate the quality of a MDS representation there are two goodness of fit measures.<!--D-->
--

- As a rule of thumb, values greater than 0.8 are desirable.<!--D-->
--

- These can be obtained using the option `eig=TRUE` in the `cmdscale` function<!--D-->
--
...
- In practice non-Euclidean distances can also be used in classical multidimensional scaling.<!--D-->
--

- However, using non-Euclidean distances can lead to negative eigenvalues.  In this case:<!--D-->
--
...
- MDS is not equivalent to PCA when $\delta$ is non-Euclidean.<!--D-->
--
...
- We want to create a 2-dimensional map with the locations of the cities using only these road distances.  However<!--D-->
--

  + The earth is 3D<!--D-->
--

  + The roads are not straight lines, in particular some costal roads may be very wiggly.  Road distances are non-Euclidean.<!--D-->
--
...
```{r ozmap,eval=TRUE,echo=FALSE,message=FALSE, warning=FALSE}
oz<-get_map(location='Australia',zoom=4,maptype = "watercolor",color = "color")
...
- If we rotate the points then the distances between the points do not change.<!--D-->
--

- Once we have an MDS solution any 2D rotated solution will be equally good.<!--D-->
--

- Sometimes these rotations help us to interpret the axes.<!--D-->
--
...
--
  ...
  + This also implies that classical MDS may not minimise strain.<!--D-->
--
...
- If you continue to read the paper you will notice that there are references to some new terms.<!--D-->
--

  + SYSTAT (a software program)<!--D-->
--

  + Non-metric MDS<!--D-->
--

  + Kruskal's algorithm<!--D-->
--

- So far we have only talked about one type of MDS, namely classical MDS.<!--D-->
--
...
- This penalises errors of observations that are close to one another more.<!--D-->
--
...
- In this case we solve a different optimisation problem.<!--D-->
--

$$\mbox{Stress}=\sum\limits_{i=1}^{n-1}\sum\limits_{j>i}\frac{(\hat{\delta}_{ij}^2-d_{ij}^2)}{\hat{\delta}_{ij}}$$<!--D-->
--

- Subject to constraints, e.g.<!--D-->
--
...
- Our solution requires us to find $\hat{\delta}$'s and $d$'s<!--D-->
--
...
- Go from a general distance matrix to a 2D representation that preserves distances as best as possible<!--D-->
--

- Focus on classical MDS with Euclidean distances<!--D-->
--

  + Minimises strain<!--D-->
--

  + Is equivalent to PCA<!--D-->
--

  + Can work well non-Euclidean distances<!--D-->
--

- Be aware of other approaches<!--D-->
--
